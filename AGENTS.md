...
Your crew consists of **16 skills** (in `.agents/skills/`) and **8 core agents + 2 custom agents** (in `.codex/agents/`). Your agent platform auto-loads both at session start.

The 8 core agents are:

`architect`, `scribe`, `sorter`, `seeker`, `connector`, `librarian`, `transcriber`, `postman`

Custom agents and custom skills created for this fork are also valid. Check `.codex/references/agents-registry.md` for the full list of active agents and skills.
...
- **Skills** handle complex, multi-step, or conversational flows. In Codex, the root context follows the relevant skill instructions directly so multi-turn state stays in one chat.
- **Agents** handle reactive, bounded tasks. In Codex, the root context may spawn a bounded child agent when delegation is worth it, but orchestration decisions stay in the root context because `agents.max_depth = 1`.
...
2. **Check skill routing FIRST** — if the user's message matches a skill trigger, follow the skill instructions directly in the root context. Do not spawn a bounded child agent for skill-routed triggers.
3. **Fall through to agent routing** — if no skill matches, use the agent routing table and spawn a bounded child agent from the root context when useful.
...
Skills handle complex, multi-step flows. **Check this table BEFORE the agent table.** If a match is found, follow the skill instructions directly in the root context and stop — do not also spawn a bounded child agent.
...
| 11+ | **custom agents** | Any additional agent created via the Architect. Check `.codex/references/agents-registry.md` for triggers and capabilities. Custom agents always have lower priority than core 8 and explicit fork custom agents. |
...
Additional custom agents are created via the `/create-agent` skill and stored in `.codex/agents/`. They are auto-discovered like core agents. When a user message does not match any skill or core agent, check `.codex/references/agents-registry.md` for custom agents whose Input column matches the message. If a match is found, delegate to that agent.
...
3. When invoking the next agent, pass the chain and position, e.g.: `"Root context history: [scribe, architect]. If another bounded child task is still useful, the root context decides whether to spawn it within `agents.max_depth = 1`."`
...
- **Child depth: `agents.max_depth = 1`**: spawned child agents do one bounded task, then the root context decides the next step
...
USER MESSAGE → check SKILL routing table first
           ↓
  Skill match found? → FOLLOW skill instructions in the root context → RESPOND to user
           ↓ (no skill match)
  Check AGENT routing table → SPAWN a bounded child agent from the root context when useful
           ↓
     READ OUTPUT → check agents-registry.md
           ↓
  Does output match another agent's capabilities?
     YES + still useful within `agents.max_depth = 1` → root context decides whether another bounded child task is warranted
...
├── .codex/config.toml            ← Gmail + Calendar (optional, if you chose yes)
...
Each agent is defined in `.codex/agents/{name}.toml` (in the destination vault) as TOML with metadata and developer instructions. The platform auto-discovers these agents at session start, reads their `description` field, and delegates automatically when the user's message matches.
...
- **Postman** uses email (Gmail via `gws`, Hey.com via `hey` CLI) and Google Calendar for full read/write access, with MCP servers (`.codex/config.toml`) as a read-only fallback. See `docs/gws-setup-guide.md` for GWS setup
